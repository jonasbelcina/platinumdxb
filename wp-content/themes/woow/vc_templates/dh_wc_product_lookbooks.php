...
?>
<?php if($lookbooks = get_terms('product_lookbook',array('hide_empty'=>0,'include'=> $ids,'orderby'=>'name','menu_order'=>'DESC'))):?>
	<div class="lookbooks lookbooks-<?php echo esc_attr($style) ?>">
		<?php 
		$i = 0;
		?>
		<?php foreach ((array)$lookbooks as $lookbook): $i++;?>
			<?php
					...
					<div class="lookbook-info">
						<div class="lookbook-info-wrap" style="background: url(<?php echo esc_attr($image)?>) no-repeat scroll center center">
							...
							<div class="lookbook-info-sumary">
								...
								<?php endif;?>
								<a class="btn btn-primary lookbook-action" href="<?php echo get_term_link($lookbook,'product_lookbook') ?>"><span><?php esc_html_e('Shop Now','loja')?></span></a>
							</div>
						</div>	
					</div>
