					...
					<?php while ($r->have_posts()): $r->the_post(); global $post;?>
						<li class="<?php echo esc_attr($post_col) ?>">
							<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> >
								...
								<?php else:?>
								<div class="hentry-wrap <?php if($layout_style == 'list') {echo 'row';} ?>">
									...
									<?php else: ?>
										...
										?>
									<?php endif; ?>
									<div class="entry-info <?php if($layout_style == 'list') {echo 'col-sm-8';};?>">
										<div class="info-inner">
											...
											<div class="entry-meta">
												<?php 
													dh_post_meta($show_date,$show_comment,$show_category,$show_author,true,false,null,false);  
												?>
											</div>
											<div class="entry-content">
												<?php 
												...
												?>
												<?php if($layout_style == 'list') : ?>
													<a class="read-more" href="<?php the_permalink()?>" title="<?php echo esc_attr(get_the_title())?>">
														<?php esc_html_e( 'Read more', 'woow'); ?>  <i class="fa fa-angle-right"></i>
													</a>
												<?php endif; ?>
											</div>
										</div><!--inner-->
