		...
		<?php while ($related->have_posts()): $related->the_post();global $post;?>
			<div class="related-post-item col-md-4 col-sm-6">
				...
				<div class="readmore-link">
					<a href="<?php the_permalink()?>"><?php esch_html_e("Read More", 'sitesao');?></a>
				</div>
			</div>
		<?php endwhile;?>
	</div>
...
function dh_get_search_form($ajax=true){
	$search_form = '<form method="GET" class="searchform search-ajax" action="'.esc_url( home_url( '/' ) ).'" role="form">
					...
					<input type="hidden" name="post_type" value="'.apply_filters('dh_ajax_search_form_post_type', 'post').'" />
				</form>';
	if($ajax)
...
function dh_comment_form( $args = array(), $post_id = null ) {
	...
	<div id="respond-wrap">
		<?php 
			...
			$fields =  array(
				'author' => '<div class="row"><p class="comment-form-author col-sm-4">' .'<input placeholder="'.esc_attr__('Name *','sitesao').'" id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
				'email' => '<p class="comment-form-email col-sm-4">'.'<input id="email" placeholder="'.esc_attr__('Email *','sitesao').'" name="email" type="text" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
				'url' => '<p class="comment-form-url col-sm-4"><input id="url"  placeholder="'.esc_attr__('Website','sitesao').'"  name="url" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p></div>'
			);
			$comments_args = array(
					...
					'comment_field'		   => '<p class="comment-form-comment"><textarea placeholder="'.__('Comment','sitesao').'"  class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
...
function dh_morphsearchform(){
	...
	<div class="morphsearch" id="morphsearch">
		<form class="morphsearch-form" method="get"  action="<?php echo esc_url( home_url( '/' ) ) ?>" role="form">
			<input type="search" name="s" placeholder="<?php esch_attr_e('Search...','sitesao')?>" class="morphsearch-input">
		...
		<div class="morphsearch-content<?php echo (defined( 'WOOCOMMERCE_VERSION' )  ? ' has-3colum':'') ?>">
			...
			<div class="dummy-column">
				<h2><?php esch_html_e('Product','sitesao') ?></h2>
				<?php 
				$query_args = array(
		    		'posts_per_page' => 6,
		    		'post_status' 	 => 'publish',
			...
			<?php }?>
			<div class="dummy-column">
				<h2><?php esch_html_e('Popular','sitesao') ?></h2>
				<?php 
			...
			<div class="dummy-column">
				<h2><?php esch_html_e('Recent','sitesao') ?></h2>
				<?php 
		...
		<div class="mailchimp-form-content clearfix">
			<div class="row">
				<div class="col-sm-8">
					...
					<input type="email" class="form-control"  required="required" autocomplete="off" placeholder="<?php esch_attr_e('Enter your email...','sitesao')?>" name="email">
				</div>
				<div class="col-sm-4">
					<button type="submit" class="btn btn-outline mailchimp-submit"><?php esch_html_e('Subscribe','sitesao')?></button>
				</div>
			</div>
