...
<div class="woocommerce-billing-fields">
	<?php if ( WC()->cart->ship_to_billing_address_only() && WC()->cart->needs_shipping() ) : ?>
		...
		<h3><?php esc_html_e( 'Billing &amp; Shipping', 'woow' ); ?></h3>

	<?php else : ?>

		<h3><?php esc_html_e( 'Billing Details', 'woow' ); ?></h3>
	...
	<?php if ( ! is_user_logged_in() && $checkout->enable_signup ) : ?>
		...
		<?php if ( $checkout->enable_guest_checkout ) : ?>
			...
			<p class="form-row form-row-wide create-account">
				<label for="createaccount" class="checkbox form-flat-checkbox">
					...
					<i></i>
					<?php esc_html_e( 'Create an account?', 'woow' ); ?>
				</label>
			</p>
		...
		<?php if ( ! empty( $checkout->checkout_fields['account'] ) ) : ?>
			...
			<div class="create-account">
				...
				<p><?php esc_html_e( 'Create an account by entering the information below. If you are a returning customer please login at the top of the page.', 'woow' ); ?></p>
