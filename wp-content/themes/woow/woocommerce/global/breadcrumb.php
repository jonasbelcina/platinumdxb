...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	elseif ( is_tax( 'product_brand' ) ) {
		...
		echo dh_print_string($prepend) . dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Products in &ldquo;', 'woow' ) . $queried_object->name . '&rdquo;' .'</span>'. dh_print_string($after);
	...
	elseif ( is_tax( 'product_tag' ) ) {
		...
		$queried_object = $wp_query->get_queried_object();
		echo dh_print_string($prepend) . dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Products tagged &ldquo;', 'woow' ) . $queried_object->name . '&rdquo;' .'</span>'. dh_print_string($after);
		...
		echo dh_print_string($before) .'<span typeof="v:Breadcrumb" property="v:title">'. esc_html__( 'Error 404', 'woow' ) .'</span>'. dh_print_string($after);
