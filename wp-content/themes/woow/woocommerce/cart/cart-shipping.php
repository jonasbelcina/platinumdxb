...
<tr class="shipping">
	<th><?php
		if ( $show_package_details ) {
			printf( esc_html__( 'Shipping #%d', 'woow' ), $index + 1 );
		} else {
			esc_html_e( 'Shipping', 'woow' );
		}
	?></th>
	<td>
		...
		<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'yes' ) : ?>
				...
				<p><?php esc_html_e( 'Please use the shipping calculator to see available shipping methods.', 'woow' ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p><?php esc_html_e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', 'woow' ); ?></p>

			<?php else : ?>

				<p><?php esc_html_e( 'Please fill in your details to see available shipping methods.', 'woow' ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					'<p>' . esc_html__( 'There are no shipping methods available. Please double check your address, or contact us if you need any help.', 'woow' ) . '</p>'
				); ?>
		...
		<?php if ( $show_package_details ) : ?>
			<?php
				...
				echo '<p class="woocommerce-shipping-contents"><small>' . esc_html__( 'Shipping', 'woow' ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
			?>
		<?php endif; ?>
