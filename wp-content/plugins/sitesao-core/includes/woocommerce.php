...
if ( ! class_exists( 'DH_Woocommerce' ) ) :
	class DH_Woocommerce {
		...
		public function init() {
			...
			add_filter('dh_get_theme_option', array(&$this,'dh_get_theme_option'),100,2);
		...
		public function custom_product_tabs($tabs){
			global $product, $post;
			$product_custom_tab = dh_get_post_meta('hide_custom_tab');
			if(dh_get_theme_option('woo-custom-tab',1) && empty($product_custom_tab)){
				$tabs['custom_tab'] = array(
					'title'    => dh_get_theme_option('woo-custom-tab-title','Custom Tab'),
					'priority' => apply_filters('dh_custom_tab_priority', 25),
		...
		public function dh_get_theme_option($value, $option){
			if(is_singular('product') && ($option == 'sticky-menu') && (dh_get_theme_option('single-product-style','style-1') == 'style-2' )){
				return '0';
			}
			return $value;
		}
		...
		public function _get_minicart_icon2(){
			return apply_filters('dh_woocommerce_minicart_icon','<svg xml:space="preserve" style="enable-background:new 0 0 459.529 459.529;" viewBox="0 0 459.529 459.529" y="0px" x="0px" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" version="1.1">
	<g>
		<g>
