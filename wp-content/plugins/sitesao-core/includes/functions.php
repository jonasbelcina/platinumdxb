...
function dh_instagram($username,$images_number=12,$refresh_hour){
	if (false === ($instagram = get_transient('instagram-'.sanitize_title_with_dashes($username)))) {
		...
		if ( is_wp_error( $remote ) )
			return new WP_Error( 'site_down',__( 'Unable to communicate with Instagram.', 'sitesao' ));
	
		if ( 200 != wp_remote_retrieve_response_code( $remote ) )
			return new WP_Error( 'invalid_response',__( 'Instagram did not return a 200.', 'sitesao' ));
		...
		if ( !$insta_array )
			return new WP_Error( 'bad_json', __( 'Instagram has returned invalid data.', 'sitesao' ) );
		...
		switch ( $type ) {
			...
			default:
				foreach ( $images as $image ) {
					...
					$instagram[] = array(
						'description'   => __( 'Instagram Image', 'sitesao' ),
		...
		if ( ! empty( $instagram ) ) {
			...
			set_transient( 'instagram-'.sanitize_title_with_dashes( $username ), $instagram, apply_filters( 'dh_instagram_cache_time', HOUR_IN_SECONDS*absint($refresh_hour) ) );
		}
	}
...
function dh_get_video_thumb_url($videoUrl) {
	switch (dh_detect_video($videoUrl)) {
		case 'youtube':
			$youtube_id = dh_get_youtube_id($videoUrl);
			if (!dh_is404('http://img.youtube.com/vi/' . $youtube_id. '/maxresdefault.jpg')) {
			...
			break;
		case 'dailymotion':
			$dailyMotionApi = @file_get_contents('https://api.dailymotion.com/video/' . dh_get_dailymotion_id($videoUrl) . '?fields=thumbnail_url');
			...
			break;
		case 'vimeo':
			$vimeoApi = @file_get_contents('http://vimeo.com/api/v2/video/' . dh_get_vimeo_id($videoUrl) . '.php');
			if (!empty($vimeoApi)) {
...
function dh_get_theme_option($option,$default = null){
	...
	$default = apply_filters('dh_get_theme_option_default', $default,$option);
	if(empty($option))
		return $default;
	...
	if(is_page() || (defined('WOOCOMMERCE_VERSION') && is_woocommerce())){
		if($option == 'header-style'){
			...
			if($page_value !== '' && $page_value !== null && $page_value !== array() && $page_value !== false && $page_value != '-1'){
		...
		if($option == 'footer-newsletter'){
			$page_value = dh_get_post_meta('footer_newsletter');
			if($page_value !== '' && $page_value !== null && $page_value !== array() && $page_value !== false &&  $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'footer-featured'){
			$page_value = dh_get_post_meta('footer_featured');
			if($page_value !== null && $page_value !== array() && $page_value !== false &&  $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
		if($option == 'footer-featured-style'){
			$page_value = dh_get_post_meta('footer_featured_style');
			if($page_value !== '' &&  $page_value !== null && $page_value !== array() && $page_value !== false && $page_value != '-1'){
				return apply_filters('dh_get_theme_option', $page_value, $option);
			}
		}
	...
	if(isset($dh_theme_options[$option]) && $dh_theme_options[$option] !== null && $dh_theme_options[$option] !== array() && $dh_theme_options[$option] !== false){
...
function dh_get_main_class($body = false,$get_layout = false){
	...
	if(defined('WOOCOMMERCE_VERSION')){
		...
		elseif (is_tax( 'product_brand' )){
			...
			remove_action('dh_right_sidebar','dh_get_sidebar');
			remove_action('dh_right_sidebar_extra','dh_get_extra_sidebar');
			$layout = dh_get_theme_option('woo-brand-layout','right-sidebar');
			$class = dh_get_main_col_class($layout,$body,11);
		}
		elseif (is_tax( 'product_tag' )){
			...
			remove_action('dh_right_sidebar','dh_get_sidebar');
			remove_action('dh_right_sidebar_extra','dh_get_extra_sidebar');
			$layout = dh_get_theme_option('woo-tag-layout','right-sidebar');
			$class = dh_get_main_col_class($layout,$body,11);
		}
		elseif (is_tax( 'product_lookbook' )){
			...
			remove_action('dh_right_sidebar','dh_get_sidebar');
			remove_action('dh_right_sidebar_extra','dh_get_extra_sidebar');
			$layout = dh_get_theme_option('woo-lookbook-layout','right-sidebar');
			$class = dh_get_main_col_class($layout,$body,11);
		}
...
<div class="related-post">
	<div class="related-post-title">
		<h3><span><?php echo __("Related News",'sitesao')?></span></h3>
	</div>
	<div class="row related-post-items">
		<?php while ($related->have_posts()): $related->the_post();global $post;?>
			<div class="related-post-item col-md-4 col-sm-6">
				...
				<?php dh_post_meta(true,false,false,false,true,' ',null,true)?>
				</div>
				<div class="excerpt">
				<?php 
				...
				<div class="readmore-link">
					<a href="<?php the_permalink()?>"><?php _e("Read More", 'sitesao');?></a>
				</div>
			</div>
		<?php endwhile;?>
	</div>
					...
					<header class="comment-header">
						<div class="comment-info">
							...
							<span class="ispostauthor">
								<?php _e('Author', 'sitesao'); ?>
							</span>
							<?php endif; ?>
							<span class="comment-meta">
								<time datetime="<?php echo get_comment_time('c'); ?>">
									<?php echo sprintf(__('%1$s at %2$s', 'sitesao') , get_comment_date() , get_comment_time()); ?>
								</time>
								<span class="comment-edit">
									<?php edit_comment_link('' . __('Edit', 'sitesao')); ?>
								</span>
							</span>
						</div>
						<?php if ('0' == $comment->comment_approved): ?>
							<p class="comment-pending"><?php _e('Your comment is awaiting moderation.', 'sitesao'); ?></p>
						<?php endif; ?>
					</header>
...
function dh_get_search_form(){
	...
	$search_form = '<form method="GET" class="searchform'.($ajax ?' search-ajax':'').'" action="'.esc_url( home_url( '/' ) ).'" role="form">
					<input type="search" class="searchinput" name="s" autocomplete="off" value="" placeholder="'.__( 'Search...', 'sitesao' ).'" />
					<input type="submit" class="searchsubmit hidden" name="submit" value="'.__( 'Search', 'sitesao' ).'" />
...
function dh_comment_form( $args = array(), $post_id = null ) {
	...
	<div id="respond-wrap">
		<?php 
			...
			$fields =  array(
				'author' => '<div class="row"><p class="comment-form-author col-sm-4">' . 
							'<label for="author">' . __( 'Name', 'sitesao' ) . ' <span class="required">*</span></label> ' .
							 '<input placeholder="'.__('Name *','sitesao').'" id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
				'email' => '<p class="comment-form-email col-sm-4">' .
						   '<label for="email">' . __( 'Email', 'sitesao' ) . ' <span class="required">*</span></label> ' .
						'<input id="email" placeholder="'.__('Email *','sitesao').'" name="email" type="text" class="form-control" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
				'url' => '<p class="comment-form-url col-sm-4">' .
						 '<label for="url">' . __( 'Website', 'woow' ) .'</label> '.
						 '<input id="url"  placeholder="'.__('Website','sitesao').'"  name="url" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p></div>'
			);
			$comments_args = array(
					...
					'title_reply'          => sprintf('<span>%s</span>',__( 'Leave a reply', 'sitesao' )),
					'title_reply_to'       => sprintf('<span>%s</span>',__( 'Leave a reply to %s', 'sitesao' )),
					'cancel_reply_link'    => __( 'Click here to cancel the reply', 'sitesao' ),
					'label_submit'         => __( 'Submit', 'sitesao' ),
					'submit_button'	       => '<button name="%1$s" type="submit" id="%2$s" class="%3$s" value="1">%4$s</button>',
					'comment_field'		   => '<p class="comment-form-comment"><label for="comment">' . __( 'Comment', 'sitesao' ) . '</label> <textarea placeholder="'.__('Comment','sitesao').'"  class="form-control" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
	...
	<nav class="post-navigation" role="navigation">
		<div class="row">
			...
			<?php if($prev_link):?>
				<div class="prev-post">
					<span>
					<?php echo __('Previous article','sitesao')?>
					</span>
			...
			<?php if(!empty($next_link)):?>
				<div class="next-post">
					<span>
						<?php echo __('Next article','sitesao')?>
					</span>
...
function dh_page_title($echo = true){
	...
	elseif (is_day())
	{
		$title = __('Archive for date:','sitesao')." ".get_the_time('F jS, Y');
	}
	elseif (is_month())
	{
		$title = __('Archive for month:','sitesao')." ".get_the_time('F, Y');
	}
	elseif (is_year())
	{
		$title = __('Archive for year:','sitesao')." ".get_the_time('Y');
	}
	elseif (is_search())
	{
		global $wp_query;
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$title =  $wp_query->found_posts ." ". __('search results for','sitesao').' <span class="search-query">'.esc_attr( get_search_query() ).'</span>';
			}
			else
			{
				$title =  $wp_query->found_posts ." ". __('search result for','sitesao').' <span class="search-query">'.esc_attr( get_search_query() ).'</span>';
			}
		}
		else
		{
			if(!empty($_GET['s']))
			{
				$title = __('Search results for','sitesao').' <span class="search-query">'.esc_attr( get_search_query() ).'</span>';
			}
			else
			{
				$title = __('To search the site please enter a valid term','sitesao');
			}
		}
	...
	elseif(is_tax())
	{
	    ...
	    $title = __('404 - Page not found','sitesao');
	}
	else {
		$title = get_the_title();
	}
	...
	{
		$title .= " (".__('Page','sitesao')." ".$_GET['paged'].")";
	}
...
// 		}
	...
	if(is_post_type_archive( 'portfolio' )){
		$title = esc_html(dh_get_theme_option('portfolio-archive-title',__('My Portfolio','sitesao')));
	}
	if($echo)
...
function dh_post_featured($post_id='',$post_format='',$is_shortcode = false,$is_related = false,$entry_featured_class = '',$layout = '',$is_thumb_square=false){
	$post_id  = empty($post_id) ? get_the_ID() : $post_id;
...
function dh_post_meta($show_date=true,$show_comment = true,$show_category= true,$show_author = true,$echo = true,$meta_separator= ', ',$date_format = null,$icon = false) {
	...
	if($show_date){
		...
		//$date_html .= sprintf(__('Date: ', 'sitesao'));
	...
	if($show_author){
		...
		if($icon)
			$author_html .= '<i class="fa fa-user"></i>';//__('By', 'sitesao');
		$author = sprintf(
			'<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'nicename' ) ) ),
			esc_attr( sprintf( __( 'Posts by: %s', 'sitesao'), get_the_author() ) ),
			get_the_author()
		);
		$author_html .= $author;
		$author_html .= '</span>';
	...
	if($show_category){
		...
		$categories_html .= get_the_category_list(', ');
	...
	if (comments_open()) {
		...
		if (get_comments_number() == 0) {
			$comment_title = sprintf(__('Leave a comment on: &ldquo;%s&rdquo;', 'sitesao') , get_the_title());
			$comment_number = '0 '.__('Comment', 'sitesao');
		} else if (get_comments_number() == 1) {
			$comment_title = sprintf(__('View a comment on: &ldquo;%s&rdquo;', 'sitesao') , get_the_title());
			$comment_number = '1 ' . __('Comment', 'sitesao');
		} else {
			$comment_title = sprintf(__('View all comments on: &ldquo;%s&rdquo;', 'sitesao') , get_the_title());
			$comment_number =  get_comments_number() . ' ' . __('Comments', 'sitesao');
		}
...
function dh_paginate_links_short($args = array(), $query = null){
	...
	$defaults = array(
		...
		'add_fragment' => '',
		'add_args'=>array(),
		'before' => '<div class="paginate"><div class="paginate_links"><span class="pagination-meta">'.sprintf(__("%d/%d", 'sitesao'), $paged, $max_num_pages).'</span>',
		'after' => '</div></div>',
		'echo' => true,
	);
...
function dh_morphsearchform(){
	...
	<div class="morphsearch" id="morphsearch">
		<form class="morphsearch-form" method="get"  action="<?php echo esc_url( home_url( '/' ) ) ?>" role="form">
			<input type="search" name="s" placeholder="<?php _e('Search...','sitesao')?>" class="morphsearch-input">
		...
		<div class="morphsearch-content<?php echo (defined( 'WOOCOMMERCE_VERSION' )  ? ' has-3colum':'') ?>">
			...
			<div class="dummy-column">
				...
				<a href="<?php the_permalink()?>" class="dummy-media-object">
					...
					<div>
						<h3><?php the_title()?></h3>
						<?php echo '<span>'.sprintf(__('%s Comment','sitesao'),get_comments_number()).'</span>'; ?>
					</div>
				</a>
				<?php endwhile; ?>
				<?php 
...
function dh_mailchimp_form($echo = true){
	ob_start();
	static $dh_mailchimp_form_count = 0;
	$dh_mailchimp_form_count++;
	$id = 'mailchimp-form-'.$dh_mailchimp_form_count;
	?>
	<div class="mailchimp-form-wrap">
		<form id="<?php echo esc_attr($id)?>" method="get" class="mailchimp-form" action="<?php echo home_url('/')?>">
			<?php 
			...
			<div class="mailchimp-form-result">
				...
				if($mailchimp_subscribe == -1){
					echo '<span class="error">'.__( 'Not Subscribe to Mailchimp!', 'sitesao' ).'<span>';
				}elseif ($mailchimp_subscribe == 1){
					echo '<span class="success">'.__( 'Subscribe to Mailchimp Successful!', 'sitesao' ).'</span>';
				}
				?>
			</div>
			<?php endif;?>
			<div class="mailchimp-form-content clearfix">
				...
				<label for="subscribe_email" class="hide"><?php _e('Subscribe','sitesao')?></label>
				<input type="email" id="subscribe_email" class="form-control"  required="required" autocomplete="off" placeholder="<?php _e('Enter your email...','sitesao')?>" name="email">
			
				<button type="submit" class="btn mailchimp-submit"><?php _e('sign up','sitesao')?></button>
...
function dh_landing_hero_content(){
	...
	<div class="heading-hero-content parallax-content">
		<div class="heading-hero-top-title"><?php _e('Welcome to','sitesao')?></div>
		<div class="heading-hero-title"><?php _e('Vicky Restaurant','sitesao')?></div>
		<div class="heading-hero-bottom-title"><?php _e('A premium Restaurant WordPress theme','sitesao')?></div>
