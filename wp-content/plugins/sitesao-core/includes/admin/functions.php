...
function dh_get_mailchimplist(){
	$options = array(__('Nothing Found...','sitesao'));
	if($mailchimp_api = dh_get_theme_option('mailchimp_api','')){
		...
		$lists = $api->lists();
		if ($api->errorCode){
			$options = array(__("Unable to load MailChimp lists, check your API Key.", 'sitesao'));
		}else{
			if ($lists['total'] == 0){
				$options = array(__("You have not created any lists at MailChimp",'sitesao'));
			}else{
				$options = array(__('Select a list','sitesao'));
						...
						jQuery(document).ready(function($) {
							$('.dh-meta-gallery-select').on('click',function(e){
								...
								dh_meta_gallery_frame = wp.media({
									title: '<?php echo __('Add Images to Gallery','sitesao')?>',
									button: {
										text: '<?php echo __('Add to Gallery','sitesao')?>',
									},
									library: { type: 'image' },
									multiple: true
								});
											...
											dh_meta_gallery_list.append('\
												<li data-id="' + attachment.id +'">\
													<div class="thumbnail">\
														...
														<a href="#" title="<?php echo __('Delete','sitesao')?>"><?php echo __('x','sitesao')?></a></li>\
													</div>\
												</li>'
											);
										}
							...
							?>
								<li data-id="<?php echo esc_attr( $attachment_id ) ?>">
									<div class="thumbnail">
										...
										<a title="<?php echo __('Delete','sitesao') ?>" href="#"><?php echo __('x','sitesao') ?></a>
									</div>						
								</li>
							<?php
								endif;
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dh_<?php echo esc_attr($field['id']); ?>_media_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo __( 'Select or Upload your Media', 'sitesao' ); ?>",
									button: {
										text: "<?php echo __( 'Select', 'sitesao' ); ?>"
									},
								...
								dh_<?php echo esc_attr($field['id']); ?>_media_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo __( 'Change Media', 'sitesao' ); ?>');
							...
							$('#<?php echo esc_attr($field['id']) ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']) ?>_upload').attr('value', '<?php echo __( 'Select Media', 'sitesao' ); ?>');
						...
						jQuery(document).ready(function($) {
							...
							$('#<?php echo esc_attr($field['id']) ?>_upload').on('click', function(event) {
								...
								var dh_<?php echo esc_attr($field['id']); ?>_image_frame = wp.media.frames.wp_media_frame = wp.media({
									title: "<?php echo __( 'Select or Upload your Image', 'sitesao' ); ?>",
									button: {
										text: "<?php echo __( 'Select', 'sitesao' ); ?>"
									},
									library: { type: 'image' },
								...
								dh_<?php echo esc_attr($field['id']); ?>_image_frame.on('select', function(){
									...
									$this.attr('value', '<?php echo __( 'Change Image', 'sitesao' ); ?>');
									$('#<?php echo esc_attr($field['id']); ?>_clear').css('display', 'inline-block');
								});
							...
							$('#<?php echo esc_attr($field['id']); ?>_clear').on('click', function(event) {
								var $this = $(this);
								$this.hide();
								$('#<?php echo esc_attr($field['id']); ?>_upload').attr('value', '<?php echo __( 'Select Image', 'sitesao' ); ?>');
