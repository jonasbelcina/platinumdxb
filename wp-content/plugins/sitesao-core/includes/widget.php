...
class DH_Social_Widget extends DH_Widget {
	public function __construct(){
		...
		$this->widget_description = __( "Display Social Icon.", 'sitesao' );
		$this->widget_id          = 'DH_Social_Widget';
		$this->widget_name        = __( 'Social', 'sitesao' );
		...
		$this->settings           = array(
			...
			'social' => array(
					...
					'std'   => '',
					'multiple'=>true,
					'label'=>__('Social','sitesao'),
					'desc' => __( 'Select socials', 'sitesao' ),
					'options' => array(
						'facebook'=>'Facebook',
						'twitter'=>'Twitter',
			...
			'style' => array(
				...
				'options' => array(
					...
					'square' =>  __('Square', 'sitesao' ),
					'round' =>  __('Round', 'sitesao' ),
					'outlined' =>  __('Outlined', 'sitesao' ),
				)
			),
		);
		parent::__construct();
...
class DH_Tweets extends WP_Widget {
	public function __construct() {
		parent::__construct (
			'dh_tweets', 		// Base ID
			'Recent Tweets', 		// Name
			array ('classname'=>'tweets-widget','description' => __ ( 'Display recent tweets', 'sitesao' ) )
		);
	}
	...
	public function widget($args, $instance) {
		...
		if(empty($instance['consumerkey']) || empty($instance['consumersecret']) || empty($instance['accesstoken']) || empty($instance['accesstokensecret']) || empty($instance['cachetime']) || empty($instance['username'])){
			echo '<strong>'.__('Please fill all widget settings!' , 'sitesao').'</strong>' . $after_widget;
			return;
		}
		...
		if($diff >= $crt || empty($dh_widget_recent_tweets_cache_time)){
			...
			if(!require_once(DHINC_DIR . '/lib/twitteroauth.php')){
				echo '<strong>'.__('Couldn\'t find twitteroauth.php!','sitesao').'</strong>' . $after_widget;
				return;
			}
			...
			if(!empty($tweets->errors)){
				if($tweets->errors[0]->message == 'Invalid or expired token'){
					echo '<strong>'.$tweets->errors[0]->message.'!</strong><br/>'.__('You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', 'sitesao' ) . $after_widget;
				}else{
	...
	protected function _relative_time($a=''){
		...
		if(is_numeric($d) && $d > 0) {
			...
			//if less then minute
			if($d < $minute) return sprintf(__("%s seconds ago",'sitesao'),floor($d));
			//if less then 2 minutes
			if($d < $minute * 2) return __("about 1 minute ago",'sitesao');
			//if less then hour
			if($d < $hour) return sprintf(__('%s minutes ago','sitesao'), floor($d / $minute));
			//if less then 2 hours
			if($d < $hour * 2) return __("about 1 hour ago",'sitesao');
			//if less then day
			if($d < $day) return sprintf(__("%s hours ago", 'sitesao'),floor($d / $hour));
			//if more then day, but less then 2 days
			if($d > $day && $d < $day * 2) return __("yesterday",'sitesao');
			//if less then year
			if($d < $day * 365) return sprintf(__('%s days ago','sitesao'),floor($d / $day));
			//else return more than a year
			return __("over a year ago",'sitesao');
		}
	}
	...
	public function form($instance) {
		...
		<p>
			<label>' . __ ( 'Consumer Secret' , 'sitesao' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token' , 'sitesao' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Access Token Secret' , 'sitesao' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Cache Tweets in every' , 'sitesao' ) . ':</label>
			<input type="text" name="' . $this->get_field_name ( 'cachetime' ) . '" id="' . $this->get_field_id ( 'cachetime' ) . '" value="' . esc_attr ( $instance ['cachetime'] ) . '" class="small-text" />'.__('hours','sitesao').'
		</p>
		<p>
			<label>' . __ ( 'Twitter Username' , 'sitesao' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Tweets to display' , 'sitesao' ) . ':</label>
		...
		<p>
			<label>' . __ ( 'Exclude replies', 'sitesao' ) . ':</label>
...
class DH_Post_Thumbnail_Widget extends DH_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( "Widget post with thumbnail.", 'sitesao' );
		$this->widget_id          = 'dh_widget_post_thumbnail';
		$this->widget_name        = __( 'Post Thumbnail', 'sitesao' );
		$this->cached = false;
		...
		$this->settings           = array(
				...
				'posts_per_page' => array(
					...
					'max'   => '',
					'std'   => 5,
					'label' => __( 'Number of posts to show', 'sitesao' )
				),
				'orderby' => array(
					'type'  => 'select',
					'std'   => 'date',
					'label' => __( 'Order by', 'sitesao' ),
					'options' => array(
							'latest'   => __( 'Latest', 'sitesao' ),
							'comment'  => __( 'Most Commented', 'sitesao' ),
					)
				),
				'categories' => array(
						'type'  => 'select',
						'std'   => '',
						'multiple'=>true,
						'label'=>__('Categories','sitesao'),
						'desc' => __( 'Select a category or leave blank for all', 'sitesao' ),
						'options' => $categories_options,
				),
				'hide_date' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => __( 'Hide date in post meta info', 'sitesao' )
				),
				'hide_comment' => array(
						'type'  => 'checkbox',
						'std'   => 0,
						'label' => __( 'Hide comment in post meta info', 'sitesao' )
				),
		);
		parent::__construct();
	}
		...
		if($r->have_posts()):
			...
			if ( $title )
				...
				while ($r->have_posts()): $r->the_post();global $post;
					...
					echo '<a href="'.esc_url(get_the_permalink()).'">'.get_the_post_thumbnail(null,'dh-thumbnail-square-small', array('title' => strip_tags(get_the_title()))).'</a>';
					echo '</div>';
					echo '<div class="posts-thumbnail-content">';
						...
						if(!$hide_comment){
							...
							if ( $number > 1 ) {
								$output = str_replace( '%', number_format_i18n( $number ), ( false === false ) ? __( '% Comments', 'sitesao' ) : false );
							} elseif ( $number == 0 ) {
								$output = ( false === false ) ? __( '0 Comments', 'sitesao' ) : false;
							} else { // must be one
								$output = ( false === false ) ? __( '1 Comment', 'sitesao' ) : false;
							}
...
class DH_Mailchimp_Widget extends DH_Widget {
	public function __construct(){
		...
		$this->widget_description = __( "Widget Mailchimp Subscribe.", 'sitesao' );
		$this->widget_id          = 'dh_widget_mailchimp';
		$this->widget_name        = __( 'Mailchimp Subscribe', 'sitesao' );
		$this->cached = false;
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
...
class DH_Woocommerce_Swatches_Widget extends DH_Widget{
	public function __construct(){
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget when viewing product categories width Swatches and Photo .', 'sitesao' );
		$this->widget_id          = 'dh_widget_swatches';
		$this->widget_name        = __( 'DH Woocommerce Swatches Layered Nav', 'sitesao' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			...
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'sitesao' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'sitesao' ),
				'options' => array(
					'and' => __( 'AND', 'sitesao' ),
					'or'  => __( 'OR', 'sitesao' )
				)
...
class DH_Woocommerce_Product_Sorting extends DH_Widget{
	public function __construct() {
		...
		$this->widget_name        = __( 'DH WooCommerce Product Sorting', 'sitesao' );
...
function dh_register_widget(){
	...
	if (defined('WC_SWATCHES_VERSION')){
