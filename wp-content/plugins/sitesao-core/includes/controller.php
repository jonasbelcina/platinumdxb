	...
	class DH_Controller {
		...
		public function facebook_init(){
			...
			if( !isset( $_REQUEST['fb_response'] ) || !isset( $_REQUEST['fb_user'] )) {
				die(json_encode(array('error' => __('Authenication required.', 'sitesao'))));
			}
			$redirect = '';
			...
			if( !$fb_userid ) {
				die(json_encode(array('error' => __('Please connect your facebook account.', 'sitesao'))));
			}
			...
			if( !$user_ID ){
				...
				if( !$user_ID )
				{
					if ( !get_option( 'users_can_register' )) {
						die( json_encode( array( 'error' => __('Registration is not open at this time. Please come back later.', 'sitesao') )));
					}
					if (dh_get_theme_option('facebook_login',0) == 0) {
						die( json_encode( array( 'error' => __('Registration using Facebook is not currently allowed. Please use our Register page', 'sitesao') )));
					}
					extract( $fb_user );
					$display_name = $name;
					...
					if( empty( $verified ) || !$verified ) {
						die(json_encode(array('error' => __('Your facebook account is not verified. You have to verify your account before proceed login or registering on this site.', 'sitesao'))));
					}
			
					$user_email = $email;
					if ( empty( $user_email )) {
						die(json_encode(array('error' => __('Please click again to login with Facebook and allow the application to use your email address', 'sitesao'))));
					}
			
					if( empty( $name )) {
						die(json_encode(array('error' => 'empty_name', __('We didn\'t find your name. Please complete your facebook account before proceeding.', 'sitesao'))));
					}
			...
			die( json_encode( array( 'loggedin' => true, 'type' => $logintype, 'url' => $redirect, 'siteUrl' => home_url(), 'message' => __( 'Login successful, redirecting...','sitesao' ) )));
		...
		public function login(){
			...
			if ( is_wp_error( $user_signon ) ){
				...
				wp_send_json(array('loggedin'=>true, 'redirecturl' => $redirecturl, 'message'=> '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __( 'Login successful, redirecting...','sitesao' ) . '</span>' ));
			}
			die();
		}
		public function register(){
			if( !check_ajax_referer('dh-ajax-register-nonce', 'security', false) ) {
				$result = array(
					'success' => false,
					'message' => '<span class="error-response">'.__( 'Your session is expired or you submitted an invalid form.', 'sitesao' ).'</span>',
				);
				wp_send_json($result);
			}
			if(get_option( 'users_can_register' )){
				...
				if ( is_wp_error( $errors ) ) {
					...
					$result = array(
						'success'     => true,
						'message'	=> '<span class="success-response"><i class="fa fa-check-circle"></i> '.__( 'Registration complete.', 'sitesao' ).'</span>',
				...
				$result = array(
					'success' => false,
					'message'   =>__( 'Not allow register in site.', 'sitesao' ),
				);
			}
			wp_send_json($result);
		}
		...
		protected function _register_new_user($user_login, $user_email, $user_password='', $cuser_password=''){
			...
			if ( $sanitized_user_login == '' ) {
				$errors->add( 'empty_username', __( 'Please enter a username.', 'sitesao' ) );
			} elseif ( ! validate_username( $user_login ) ) {
				$errors->add( 'invalid_username', __( 'This username is invalid because it uses illegal characters. Please enter a valid username.', 'sitesao' ) );
				$sanitized_user_login = '';
			} elseif ( username_exists( $sanitized_user_login ) ) {
				$errors->add( 'username_exists', __( 'This username is already registered. Please choose another one.', 'sitesao' ) );
			}
			
			// Check the email address
			if ( $user_email == '' ) {
				$errors->add( 'empty_email', __( 'Please type your email address.', 'sitesao' ) );
			} elseif ( ! is_email( $user_email ) ) {
				$errors->add( 'invalid_email', __( 'The email address isn\'t correct.', 'sitesao' ) );
				$user_email = '';
			} elseif ( email_exists( $user_email ) ) {
				$errors->add( 'email_exists', __( 'This email is already registered, please choose another one.', 'sitesao' ) );
			}
			//Check the password
			...
			if(empty($user_password)){
				...
				if(strlen($user_password) < 6){
					$errors->add( 'minlength_password', __( 'Password must be 6 character long.', 'sitesao' ) );
				}elseif (empty($cuser_password)){
					$errors->add( 'not_cpassword', __( 'Not see password confirmation field.', 'sitesao' ) );
				}elseif ($user_password != $cuser_password){
					$errors->add( 'unequal_password', __( 'Passwords do not match.', 'sitesao' ) );
				}
			}
			...
			if ( ! $user_id ) {
				$errors->add( 'registerfail', __( 'Couldn\'t register you... please contact the site administrator', 'sitesao' ) );
				return $errors;
			}
			...
			// user email
			$subject = sprintf(__('Welcome to [%1$s]','sitesao'),$blogname);
			$to = $user->email;
...
%4$s','sitesao');
		...
		public function register_priv(){
			...
			wp_send_json(array('success'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','sitesao'),$link)));
			die();
		}
		
		public function login_priv(){
			$link = "javascript:window.location.reload();return false;";
			wp_send_json(array('loggedin'=>true, 'message'=> sprintf(__('You are already logged in. Please <a href="#" onclick="%s">refresh</a> page','sitesao'),$link)));
			die();
		}
		...
		public function lostpassword(){
			...
			if ( isset($_POST) ) {
				...
				if ( empty( $_POST['user_login'] ) ) {
					$errors->add('empty_username', __('<strong>ERROR</strong>: Enter a username or e-mail address.', 'sitesao'));
				} else if ( strpos( $_POST['user_login'], '@' ) ) {
					$user_data = get_user_by( 'email', trim( $_POST['user_login'] ) );
					if ( empty( $user_data ) )
						$errors->add('invalid_email', __('<strong>ERROR</strong>: There is no user registered with that email address.'. 'sitesao'));
				} else {
				...
				if ( !$user_data ) {
					$errors->add('invalidcombo', __('<strong>ERROR</strong>: Invalid username or e-mail.', 'sitesao'));
					echo '<span class="error-response">' . $errors->get_error_message() . '</span>';
					die();
				}
				...
				*/
				...
				if ( ! $allow ) {
					echo '<span class="error-response">' . __('Password reset is not allowed for this user', 'sitesao') . '</span>';
					die();
				}
				...
				*/
				...
				$message = __('Someone requested that the password be reset for the following account:', 'sitesao') . "\r\n\r\n";
				$message .= network_home_url( '/' ) . "\r\n\r\n";
				$message .= sprintf(__('Username: %s', 'sitesao'), $user_login) . "\r\n\r\n";
				$message .= __('If this was a mistake, just ignore this email and nothing will happen.', 'sitesao') . "\r\n\r\n";
				$message .= __('To reset your password, visit the following address:', 'sitesao') . "\r\n\r\n";
				...
				*/
				...
				$title = sprintf( __('[%s] Password Reset', 'sitesao') );
				...
				*/
				...
				if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
					echo '<span class="error-response">' . __("Failure!", 'sitesao');
					echo __('The e-mail could not be sent.', 'sitesao');
					echo "</span>";
					die();
				} else {
					echo '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __("Email successfully sent!", 'sitesao')."</span>";
					die();
				}
			}
			die();
		...
		protected function _mailchimp_subscribe($is_ajax = false) {
			...
			if(!wp_verify_nonce( $nonce, 'mailchimp_subscribe_nonce' )){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Verify Nonce Failed','sitesao') . '</span>' ));
				else
					wp_die(__('Verify Nonce Failed','sitesao'));
			}
			if(!filter_var(@$data['email'], FILTER_VALIDATE_EMAIL)){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Please enter a valid email address','sitesao') . '</span>' ));
				else
					wp_die(__('Invalid Email','sitesao'));
			}
			...
			if($is_ajax){
				if($success == 1){
					wp_send_json(array( 'success' => true, 'message' => '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __('<strong>Well done!</strong> Subscribe to our Newsletter successful!','sitesao') . '</span>' ));
				}else{
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Can not subscribe our Newsletter','sitesao') . '</span>' ));
				}
			}else{
