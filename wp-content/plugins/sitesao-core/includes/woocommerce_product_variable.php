...
class DH_Woocommerce_Product_Variable {
	...
	public function add_product_fields() {
		...
		woocommerce_wp_checkbox(
			array(
				...
				'label'         => __('Disable Variation Gallery ?', 'sitesao')
			)
		);
		echo '</div>';
	...
	public function admin_enqueue_scripts(){
		if(get_post_type() == 'product'){
			...
			wp_enqueue_script('dh-woocommerce-product-variable',DHINC_ASSETS_URL.'/js/admin-product-variable.js',array('jquery','wc-admin-product-meta-boxes'),DHINC_VERSION,true);
		}
	}
		...
		if(!empty($images_ids)){
			...
			if($image_id == 'placeholder'){
				...
				if(!array_key_exists($image_id, $images)){
					...
					$images[] = array(
						'full' => $this->__get_attachment_image_src($image_id, 'dh-full'),
