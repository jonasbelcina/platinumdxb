		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if(is_numeric(get_query_var('w') && 0 !== get_query_var('w') ))
					$item['last'] =  sprintf( __('Weekly Archive for: "%s"','sitesao'),get_the_time('W'));
				elseif ( is_day() )
				$item['last'] = sprintf( __('Daily Archive for: "%s"','sitesao'),get_the_time('F jS, Y'));
				elseif ( is_month() )
				$item['last'] =  sprintf( __('Monthly Archive for: "%s"','sitesao'),get_the_time('F jS, Y'));
				elseif ( is_year() )
				$item['last'] =  sprintf(__('Yearly Archive for: "%s"','sitesao'),get_the_time('Y'));
			}
		
			else if ( is_author() )
				$item['last'] =  sprintf(__('Author Archive for: "%s"','sitesao'),get_the_author_meta( 'display_name', $wp_query->post->post_author ));
		}
		
		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] =  __('Search','sitesao');//sprintf(__('search Results for: "%s"','sitesao'),stripslashes( strip_tags( get_search_query() ) ));
		
		/* If viewing a 404 error page. */
		else if ( is_404() )
			$item['last'] = __( 'Page Not Found', 'sitesao' );
	...
	protected function _bbpress_items() {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() )
				$item[] = __( 'Split', 'sitesao' );
	
			elseif ( bbp_is_topic_merge() )
			$item[] = __( 'Merge', 'sitesao' );
	
			elseif ( bbp_is_topic_edit() )
			$item[] = __( 'Edit', 'sitesao' );
		}
