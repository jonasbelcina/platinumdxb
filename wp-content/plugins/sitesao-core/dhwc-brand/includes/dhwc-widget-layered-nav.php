...
class DHWC_Widget_Layered_Nav extends WP_Widget {
	...
	public function __construct(){
		$widget_options = array( 'classname' => 'woocommerce widget_layered_nav dhwc_widget_layered_nav', 'description' => __( 'Shows product brands in a widget which lets you narrow down the list of products when viewing product categories','sitesao') );
		$control_options = array();
		parent::__construct('dhwc_widget_layered_nav',__('DHWOO Product Brand Layered Nav','sitesao'), $widget_options, $control_options);
	}
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . __( 'Any brand', 'sitesao' ) .'</option>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) )
			$new_instance['title'] = __('Brands','sitesao');
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', 'sitesao' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', 'sitesao' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', 'sitesao' ); ?></option>
		</select></p>
