...
function dhwc_product_brand_description() {
	echo wpautop( __( 'Product brands for your store can be managed here. To change the order of brands on the front-end you can drag and drop to sort them. To see more brands listed click the "screen options" link at the top of the page.', 'sitesao' ) );
}
...
?>
	<div class="form-field">
		...
		<div style="line-height:60px;">
			...
			<button type="submit" class="upload_image_button button"><?php _e('Upload/Add image', 'sitesao'); ?></button>
			<button type="submit" class="remove_image_button button"><?php _e('Remove image', 'sitesao'); ?></button>
		</div>
...
function dhwc_edit_brand_thumbnail($term, $taxonomy ){
	...
	?>
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e('Thumbnail', 'sitesao'); ?></label></th>
		<td>
...
function dhwc_product_brand_columns( $columns ) {
	...
	$new_columns['thumb'] = __( 'Image', 'sitesao' );
...
function dhwc_product_brand_get_admin_settings(){
	return  apply_filters( 'dhwc_product_brand_settings_fields', array(
		array(
			'name' => __( 'Brands Options', 'sitesao' ),
			'type' => 'title',
			'id' => 'product_brand' ),
		array(
			'name' 		=> __( 'Show description', 'sitesao' ),
			'desc' 		=> __( 'Show/hide the product brand description on the archive page.', 'sitesao' ),
...
function dhwc_permalink_settings_add(){
	add_settings_field(
		...
		__( 'Product brand base', 'sitesao' ), 	// setting title
...
function dhwc_product_brand_slug_input(){
	$permalinks = get_option( 'woocommerce_permalinks' );
	?>
	<input name="dhwc_product_brand_slug" type="text" class="regular-text code" value="<?php if ( isset( $permalinks['brand_base'] ) ) echo esc_attr( $permalinks['brand_base'] ); ?>" placeholder="<?php echo _x('product-brand', 'slug', 'sitesao') ?>" />
	<?php
}
...
function dhwc_edit_product_columns($existing_columns){
	...
	foreach ($existing_columns as $key=>$column){
		if($key=='product_cat'){
			$columns[$key] =$column;
			$columns['product_brand'] = __( 'Brands', 'sitesao' );
		}else{
			$columns[$key]=$column;
		}
	}
