...
class DHWC_Widget_Brand_Slider extends WP_Widget {
	...
	public function __construct(){
		$widget_options = array( 'classname' => 'dhwc_widget_brand_slider', 'description' => __( 'Shows product brans with slider','sitesao') );
		$control_options = array();
		parent::__construct('dhwc_widget_brand_slider',__('DHWOO Product Brand Slider','sitesao'), $widget_options, $control_options);
		
		if (!is_admin()){
			add_action("wp_print_styles", array(&$this, 'enqueue_scripts'));
			wp_enqueue_script('dhwc-widget-brand-slider',WP_PLUGIN_URL.'/'.plugin_basename(dirname(dirname(__FILE__))).'/assets/js/jquery.bxslider.min.js',array('jquery'),'',true);
		}
		
	}
	
	function enqueue_scripts(){
		wp_enqueue_style('dhwc-widget-brand-slider',WP_PLUGIN_URL.'/'.plugin_basename(dirname(dirname(__FILE__))).'/assets/css/jquery.bxslider.css');
	...
	public function widget($args, $instance){
		...
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __( 'Brands', 'sitesao' ) : $instance['title'], $instance, $this->id_base);
	...
	public function form($instance){
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'sitesao') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e( 'Order by:', 'sitesao' ) ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('hide_empty'); ?>"><?php _e( 'Hide empty brands', 'sitesao' ); ?></label>
		</p>
		<?php
	}
}