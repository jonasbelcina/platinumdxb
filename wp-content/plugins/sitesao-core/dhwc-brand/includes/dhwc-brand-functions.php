...
function dhwc_product_dropdown_brands($show_counts = 1, $hierarchical = 1, $show_uncategorized = 1, $orderby = ''){
	...
	$output .= '<option value="" ' .  selected( isset( $_GET['product_brand'] ) ? $_GET['product_brand'] : '', '', false ) . '>'.__( 'Select a brand', 'sitesao' ).'</option>';
	$output .= woocommerce_walk_category_dropdown_tree( $terms, 0, $r );

	if ( $show_uncategorized )
		$output .= '<option value="0" ' . selected( isset( $_GET['product_brand'] ) ? $_GET['product_brand'] : '', '0', false ) . '>' . __( 'Uncategorized', 'sitesao' ) . '</option>';
...
function dhwc_list_brands( $args = '' ){
	$defaults = array(
		'show_option_all' => '', 'show_option_none' => __('No brands', 'sitesao'),
		...
		'hierarchical' => true, 'title_li' => __( 'Brands', 'sitesao' ),
		'echo' => 1, 'depth' => 0,
